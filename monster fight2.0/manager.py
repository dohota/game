...
class Page:
    ...
    def __init__(self):
        self.screen = pygame.display.set_mode((Page.sx,Page.sy))
        self.sound = Sound()
    ...
    def main(self):
        ...
        self.screen.fill(Page.START_BACK)
        self.write('1 player',400, 20)
        self.write('2 player', 400, 200)
        self.write('2 player socket',400, 300)
        self.write('introduction the game', 400, 400)
        while 1:
            time.sleep(0.1)
...
class Logic:
    ...
    def start(self):
        self.sound.back()
        self.player()
        self.maps.map1(random.randint(2,10),random.randint(0,2),'bedrock',-1)
        ...
        while 1:
            ...
            #中文无法显示
            self.write('player life:' + str(self.life), 20, 20)
            self.write('it is ' + str(Logic.j) + ' round', 20, 50)
            self.write('The game has been ' + str(self.timer) + ' second', 20,80)
            self.write('have ' + str(self.kill) + ' kill', 20, 110)
            for i in self.players:
                i.control()
                i.display()
            for j in self.enemys:
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    self.enemy()
                    self.maps.map1(2, -1,self.rann[random.randint(0,3)] ,random.randint(1,9))
