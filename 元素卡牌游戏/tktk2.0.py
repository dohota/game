...
class Game:
    ...
    def loop(self, s):
        ...
        if Game.enemy_action == 2 and Game.a == 0:
            ...
            s.master.quit()
        if Game.a != -1:
            p1 = tkinter.Label(s.master, text="Your turn:"+["energy", "defend", "attack"][Game.a])
            p1.pack()
            p2 = tkinter.Label(s.master, text="Enemy turn:"+["energy", "defend", "attack"][Game.enemy_action])
            p2.pack()
            time.sleep(1)
            s.clean_screen()
        s.master.after(350, self.loop(s))
...
if __name__ == "__main__":
    ...
    # master是其父窗口
    app = Window(master=tkinter.Tk())
    # 创建线程并启动
    g = Game()
    app.mainloop()
    thread = threading.Thread(target=g.loop(app))
    thread.start()