...
public:
	...
	void changel(float progress){
		...
		music.Play(this->playtime);//单位是毫秒
	}
	...
	void start(){
		...
		this->index = 0;
		std::function<void()> callback1 = [this]() { this->last_song(); };
		std::function<void()> callback2 = [this]() { this->change_pause(); };
		std::function<void()> callback3 = [this]() { this->next_song(); };
		std::function<void()> callback4 = [this]() { this->add_volume(); };
		std::function<void()> callback5 = [this]() { this->minus_volume(); };
